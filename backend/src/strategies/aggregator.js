...
const { detectRegime, detectRegimeStateless } = require('../engine/regimeDetector');
...
function aggregate(prices, opts = {}) {
  ...
  const symbol         = opts.symbol         || null;     // for stateful regime per symbol
  const useRegime      = opts.useRegime       !== false;  // default ON
  const overrideWeights = opts.overrideWeights || null;

  // ── Regime-aware weight selection ────────────────────────────────────────
  // If overrideWeights provided: use as-is (caller controls weights)
  // If symbol + useRegime: stateful detectRegime (with hysteresis)
  // If useRegime but no symbol: stateless detectRegime (no side-effects)
  // Otherwise: default config weights
  let regimeResult = null;
  let activeWeights = overrideWeights || WEIGHTS;

  if (!overrideWeights && useRegime && prices.length >= 60) {
    regimeResult  = symbol
      ? detectRegime(prices, symbol)
      : detectRegimeStateless(prices);
    activeWeights = regimeResult.weights || WEIGHTS;
  }
